...
Changelogs.parse = async function () {
    const changelogUrls = await getChangelogUrls();
    const fullChangelog = (await Promise.all(changelogUrls.map(await CachedDownload.download))).join('\n');
    return parseChangelog(fullChangelog);
};

/**
 * @return {Promise<string[]>}
 */
async function getChangelogUrls() {
    /**
     * @type {ChangeLog[]} changelogs
    ...
    return folderContent.reduce((urls, fileOrFolder) => {
        if (fileOrFolder.type === 'file' && filenamePattern.test(fileOrFolder.name)) {
            console.debug('Changelog URL found', fileOrFolder.name);
            urls.push(fileOrFolder.download_url);
        } else {
            console.debug('Excluding changelog', fileOrFolder.type, fileOrFolder.name);
        }
        return urls;
    }, []);
}
    ...
    for (const line of changelog.split('\n')) {
        ...
        for (const cveMatch of cveMatches) {
            try {
                ...
                console.warn('Unable to add CVE from line:', line);
            }
        }
    }

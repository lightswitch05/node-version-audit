...
/**
 * @typedef SupportEndDate
 * @type {object}
 * @property {Date} start
 * @property {?Date} lts
 * @property {Date} maintenance
 * @property {Date} end
 */
...
async function loadRawRules(update) {
    if (update) {
        try {
            ...
            console.warn('Unable to download hosted rules', e);
        }
    }
...
Rules.saveRules = (releases, cves, supportSchedule) => {
    const latestRelease = Object.values(releases).sort(NodeRelease.sort)[Object.keys(releases).length - 1];
    const rules = {
        lastUpdatedDate: new Date().toISOString(),
        name: 'Node Version Audit',
        website: 'https://github.com/lightswitch05/node-version-audit',
        licence: 'https://github.com/lightswitch05/node-version-audit/blob/master/LICENSE',
...
Rules.assertFreshRules = (rules) => {
    const elapsedSeconds = new Date().getSeconds() - rules.lastUpdatedDate.getSeconds();
...
function transformRules(rules) {
    ...
    for (let supportVersion in rules.supportEndDates) {
        for (let property in rules.supportEndDates[supportVersion]) {
            rules.supportEndDates[supportVersion][property] = new Date(rules.supportEndDates[supportVersion][property]);
        }
    }

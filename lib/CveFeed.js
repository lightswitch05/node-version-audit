...
CveFeed.parse = async (cveIds) => {
    ...
    for (let feedName of feedNames) {
        cveDetails = cveDetails.concat(await CveFeed._parseFeed(cveIds, feedName));
    }
    return cveDetails.sort(CveDetails.compare).reduce((sortedCveDetails, cveDetail) => {
